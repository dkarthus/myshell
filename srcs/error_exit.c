...
void	str_error(t_inst *inst, const char *str)
{
...
int		error_exit(t_inst *inst, int err_code)
{
	...
	if (err_code == -1)
		str_error(inst, "chdir() returned -1\ncould not change current working directory\n");
	else if (err_code == -2)
		str_error(inst, "getcwd() returned NULL\ncould not get "
				  "current working directory\n");
	else if (err_code == -3)
		str_error(inst, "ft_add_env_value(old_pwd, inst->env_head) "
					   "returned 0\ncould not add env variable\n");
	else if (err_code == -4)
		str_error(inst, "ft_add_env_value(pwd, inst->env_head)"
							 "returned 0\ncould not add env variable\n");
	else if (err_code == -5)
	{
