...
char	**ft_combine(char *name, char **spl_path)
{
	...
	while (spl_path[i])
	{
		...
		free(tmp);
		if (!spl_path[i])
			return (ft_free("\033[90mMalloc error\033[0m", spl_path));
		i++;
	}
	free(spl_path[i - 1]);
...
static char	*ft_find_bin(char **path_combos)
{
	...
	{
		g_exit_status = 127;
		ft_free("\033[90mminishell: command not found\033[0m", path_combos);
		return (0);
	}
	path = ft_strdup(path_combos[i]);
	if (!path)
	{
		ft_free("\033[90mMalloc error in bin.finder\033[0m", path_combos);
		return (0);
	}
...
char	*ft_get_bin_path(char *name, t_env **head)
{
	...
	if (!path || !*path)
	{
		printf("\033[90mPlease set PATH variable\033[0m\n");
		return (0);
	}
	split_path = ft_split(path, ':');
	if (!split_path)
	{
		printf("\033[90mMalloc error\033[0m\n");
		return (0);
	}
