...
int	if_for_unset_1(t_inst *inst, t_u_e *u, char *arg, char *next_arg)
{
	if (unset_env_name(arg) == u->tilde_minus)
		exit_status = unset_tilde_minus(inst, arg);
	else if (unset_env_name(arg) == u->tilde_minus_slash)
		exit_status = unset_tilde_minus_slash(inst, arg);
	else if (unset_env_name(arg) == u->tilde_minus_slash_s)
		exit_status = unset_tilde_minus_slash_s(inst, arg);
	else if (unset_env_name(arg) == u->tilde_plus)
		exit_status = unset_tilde_plus(inst, arg);
	else if (unset_env_name(arg) == u->tilde_plus_slash)
		exit_status = unset_tilde_plus_slash(inst, arg);
	else if (unset_env_name(arg) == u->tilde_plus_slash_s)
		exit_status = unset_tilde_plus_slash_s(inst, arg);
...
int	unset(t_inst *inst, char **args)
{
	...
	if (args[1] != NULL)
	{
		while (args[u.i] != NULL)
		{
			exit_status = if_for_unset(&u, args[u.i]);
			if (exit_status == 0)
				return (exit_status);
			exit_status = if_for_unset_1(inst, &u, args[u.i], args[u.i]);
			if (exit_status == 0)
				return (exit_status);
			u.i++;
		}
